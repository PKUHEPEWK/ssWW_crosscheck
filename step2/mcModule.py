...
class mcProducer(Module):
	...
	def analyze(self, event):
		...
		if sign(leptons[0].pdgId)+sign(leptons[1].pdgId) == 0: return False
        	if leptons[0].pt < 30 or leptons[1].pt < 30: return False
		...
		vetoleptons = Collection(event, "VetoLepton")
		if len(vetoleptons) > 0 and vetoleptons[0]>10: return False
		...
		if event.mll<20: return False
		if event.dmZll_veto<15: return False

		jets = Collection(event, "Jet")
		for ijet in range(1,len(jets)):
			if jets[ijet-1].pt > 20 and jets[ijet-1].btagCSVV2 > 0.8484: return False

		taus = Collection(event, "Tau")
		for itau in range(1,len(taus)):
			if taus[itau-1].pt > 18 and taus[itau-1].rawIso>=1 and math.sqrt((taus[itau-1].eta-leptons[0].eta)*(taus[itau-1].eta-leptons[0].eta) + (abs(abs(taus[itau-1].phi - leptons[0].phi) - math.pi)- math.pi)*(abs(abs(taus[itau-1].phi - leptons[0].phi) - math.pi)- math.pi)) >=0.3 and math.sqrt((taus[itau-1].eta-leptons[1].eta)*(taus[itau-1].eta-leptons[1].eta) + (abs(abs(taus[itau-1].phi - leptons[1].phi) - math.pi)- math.pi)*(abs(abs(taus[itau-1].phi - leptons[1].phi) - math.pi)- math.pi)) >=0.3:return False
